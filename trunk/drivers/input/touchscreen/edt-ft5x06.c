...
static  int  edt_ft5x06_read_reg(struct i2c_client *client,
				   u16 wr_len, u8 *wr_buf,
				   u16 rd_len, u8 *rd_buf)
{
     int len, sef_len;
	...
	while (len > 0)
	{
	    edt_ft5x06_ts_readwrite(client,1,wr_buf, 4,rd_buf);
		i += 4;
		*wr_buf = *wr_buf + i;
		rd_buf +=  i;
		 len --; 
	...
	if (sef_len)
	{
		edt_ft5x06_ts_readwrite(client,1,wr_buf,sef_len,rd_buf);
	}
...
static void edt_ft5x06_ts_wokeque(struct work_struct *work)
{

	struct edt_ft5x06_ts_data *tsdata =
			container_of(work, struct edt_ft5x06_ts_data, pen_event_work);
					...
					char *fw_version)
{
	...
	memset(rdbuf, 0, sizeof(rdbuf));
	rdbuf[0] = 0xbb;
	error = edt_ft5x06_ts_readwrite(client, 1, &rdbuf[0],
					EDT_NAME_LEN - 1, rdbuf);
	if (error)
		return error;
	for (i =0; i < EDT_NAME_LEN - 1; i ++)
	{
		printk("rdbuf[%d]:%x\n",i,rdbuf[i]);
					 ...
					 const struct i2c_device_id *id)
{
    ...
    INIT_WORK(&tsdata->pen_event_work, edt_ft5x06_ts_wokeque);  
	...
	irq_flags = IRQF_TRIGGER_FALLING |IRQF_ONESHOT;
	 ...
	 tsdata->irq  =gpiod_to_irq(tsdata->irq_gpio);
	 gpiod_set_debounce(tsdata->irq_gpio,10);

   printk("tsdata->irq:%d,gpio:%d\n",tsdata->irq,desc_to_gpio(tsdata->irq_gpio));
	error = devm_request_threaded_irq(&client->dev, tsdata->irq ,
...
static const struct of_device_id edt_ft5x06_of_match[] = {
	{ .compatible = "edt,edt-ft5206_base", .data = &edt_ft5x06_data },
