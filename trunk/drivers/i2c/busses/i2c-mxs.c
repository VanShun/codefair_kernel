...
static void mxs_i2c_derive_timing(struct mxs_i2c_dev *i2c, uint32_t speed)
{
	...
	if (speed=100000)
	{
	i2c->timing0 = 0x00780030;
	i2c->timing1 = 0x00800030;
	i2c->timing2 = 0x0015000d;
	}
	else{
	i2c->timing0 = 0x000F0007;
    // i2c->timing0 = 0x00780030;
	i2c->timing1 = 0x001F000F;
	i2c->timing2 = 0x0015000d;
	}
	printk("new i2c\n");
#endif
...
{
	const struct of_device_id *of_id =of_match_device(mxs_i2c_dt_ids, &pdev->dev);
	struct device *dev = &pdev->dev;
	struct mxs_i2c_dev *i2c;
	...
	clk_set_rate(i2c->clk_sclk, 24000000);
	printk("timing2 reg = %x\n", readl(i2c->regs + 0x30));
	printk("AP_PWR_I2CCLK_CTL clk = %x\n", clk_get_rate(i2c->clk_sclk));
	ret = clk_prepare_enable(i2c->clk_sclk);
	if (ret) {
		dev_err(&pdev->dev, "can't enable I2C sclk clock\n");
		return ret;
	...
	if (!i2c->dmach) {
		...
		//return -ENODEV;		////deleted by xieyh 200324
	}
